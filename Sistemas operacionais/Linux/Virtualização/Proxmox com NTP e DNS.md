...
Só que tem um pequeno problema nisso tudo. O que acontece no meu cenário é que o firewall distribui IP via DHCP para rede onde estou conectado, não sei mexer nas configurações de rede ainda do Proxmox, então estou tendo falha para essa minha ideia de espelhar o NTP do servidor de rede do Proxmox.

Podemos usar diversas validações para verifica se o NTP está sendo sincronizado e em todos eles fala que não foi sincronizado.