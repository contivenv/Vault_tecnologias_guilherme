...
Ou você pode usar o comando `cat ~/.ssh/id_ed25519.pub`, selecionar todo o texto (de `ssh-ed25519` até o seu e-mail) e copiar (Ctrl+C ou Cmd+C).
...
### 5. Configurar nome e e-mail para registros de alterações 

Para um histórico detalhado para saber de suas alterações, realize os seguintes comandos

```bash
git config --global user.name "Guilherme Conti Teixeira"
git config --global user.mail "guilhermect@tutanota.com"
```
